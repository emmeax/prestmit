...
const NavBar = () => {
    ...
    return <div className="flex justify-between items-start  md:gap-10 md:items-center py-10 md:mx-10 lg:mx-20 mx-10">
        ...
        <div className={`${navbar ? "flex" : "hidden"
            ...
            <p onClick={() => setNavbar(!navbar)} className="cursor-pointer">Products</p>
            <p onClick={() => setNavbar(!navbar)} className="cursor-pointer">Services</p>
            <p onClick={() => setNavbar(!navbar)} className="cursor-pointer">About</p>
            <button onClick={() => setNavbar(!navbar)} className="bg-white font-bold text-[#0A2640] rounded-full px-5 w-fit py-1 md:px-9 md:py-2 mr-5">Log In</button>
        </div>
       </div>
    </div>;
};
